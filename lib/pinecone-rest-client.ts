 ...
 */
export async function getIndexStats(options?: { filter?: Record<string, any> }) {
  console.log(`Getting index statistics from Pinecone`, {
    filter: options?.filter ? JSON.stringify(options.filter).substring(0, 100) + "..." : "none",
  })
  ...
  try {
    ...
    const url = `${host}/describe_index_stats`
    const body: any = {}
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error(`Pinecone describe_index_stats error: Status ${response.status}`, {
        statusText: response.statusText,
        error: errorText,
        host: host.split(".")[0], // Log only the first part of the host for security
      })
      throw new Error(`Pinecone describe_index_stats failed: ${response.status} ${response.statusText} - ${errorText}`)
    }

    const result = await response.json()
    console.log(`Successfully retrieved index statistics from Pinecone`, {
      totalVectorCount: result.totalVectorCount,
      namespaces: Object.keys(result.namespaces || {}),
    })
    return result
  } catch (error) {
    console.error("Pinecone describe_index_stats exception:", error)
    throw error
  }
}
